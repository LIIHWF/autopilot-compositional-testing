...
import src.semantic_model.geometry.proto.geometry_pb2 as geometry_pb2
from src.semantic_model.geometry.feature.reference_line import ReferenceLine, SLPoint
from src.semantic_model.geometry.feature.bound_box import BoundBox
...
class Polyline(ProtoClass, ReferenceLine, BoundBox):
    ...
    def get_sl_by_xy(self, x: 'Number', y: 'Number') -> Optional['SLPoint']:
        ...
        for cur_segment in self._line_segments:
            ...
            if point_sl is not None:
                ...
                LineSegment(pre_segment.v2, pre_segment.v2 + pre_segment.vec.left_unit).inner_to_left_test(point) ^
                LineSegment(cur_segment.v1, cur_segment.v1 + cur_segment.vec.left_unit).inner_to_left_test(point)
            ):
                l = pre_segment.v2.dis_to(point)
                if abs(res_l) > l or res_s is None:
                    res_l = l if pre_segment.inner_to_left_test(point) else -l
                    res_s = cum_s
    ...
    def intersect_point(self, other: 'Polyline') -> Optional[Vertex]:
        for m_line in self._line_segments:
            for o_line in other.line_segments:
                point = m_line.intersect_point(o_line)
    ...
    def get_line_segment_order_by_s(self, s: Number) -> Optional[int]:
        idx = 0
        while idx < len(self._line_segments) and s > self._line_segments[idx].length:
            s -= self._line_segments[idx].length
            idx += 1
